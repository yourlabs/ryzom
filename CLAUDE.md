...
| `ryzom_mdc` / `ryzom_django_mdc` | Material Components widgets (`html.py`) |
...
| `ryzom_example_crud` | The live Product demo (`/crud/products/`) |
...
- **Postgres is required** (Subscription uses `django.contrib.postgres` ArrayField).
  Defaults: db/user/pass all `ryzom` on `127.0.0.1:5432` (override with `DB_*`).
...
- Components subclass `Component` (or an MDC widget) and build trees from
  ...
  → use the **create-component** skill. Canonical example: `src/ryzom_example_crud/components.py`.
